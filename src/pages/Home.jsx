...
const NAME_PHRASES = ['Ravi Shyam Singh', 'Open for Collaboration']

// 🖼️ Public asset paths served from the root
const photo = '/myphoto.jpg'
...
export default function Home() {
  const [namePhraseIndex, setNamePhraseIndex] = useState(0)
  const [nameText, setNameText] = useState('')
  const [nameTypingDelay, setNameTypingDelay] = useState(120)
  ...
  useEffect(() => {
  const currentPhrase = NAME_PHRASES[namePhraseIndex]

    const timeout = setTimeout(() => {
      if (!nameDeleting) {
        const next = currentPhrase.substring(0, nameText.length + 1)
        setNameText(next)

        if (next === currentPhrase) {
          setNameDeleting(true)
          setNameTypingDelay(1200) // pause when full word is shown
        } else {
          setNameTypingDelay(120)
        }
      } else {
        const next = currentPhrase.substring(0, Math.max(0, nameText.length - 1))
        setNameText(next)

        if (next === '') {
          setNameDeleting(false)
          setNamePhraseIndex((namePhraseIndex + 1) % NAME_PHRASES.length)
          setNameTypingDelay(400) // brief pause before next phrase
        } else {
          setNameTypingDelay(80)
        }
      }
    }, nameTypingDelay)

    return () => clearTimeout(timeout)
  }, [nameDeleting, namePhraseIndex, nameText, nameTypingDelay])
        ...
        >
          ...
          >
            {[ 
              ...
              { label: '💼 Expertise', value: 'Full-stack platforms, Realtime collaboration' },
              { label: '📧 Contact', value: 'rshyamsingh106@gmail.com' },
            ].map((info, i) => (
              <motion.div
                key={i}
